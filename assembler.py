...
immed_re  = re.compile(r'''^(?P<instr>(ffo|zb|bc|ori|addi|andi))\s+(?P<rt>\$r[0,1,2,3])\s+(?P<rs>\$r[0,1,2,3])\s+(?P<immed>-?(0x)?[0-9a-fA-F]+)$''')
twoReg_re = re.compile(r'''^(?P<instr>(ffo|zb|bc|ori|addi|andi))\s+(?P<rt>\$r[0,1,2,3])\s+(?P<rs>\$r[0,1,2,3])$''')
...
signed_re = re.compile(r'addi|beq|bne|lw|sw')
...
def imm_check(signed,both_allowed,immediate,lineNo):
  if both_allowed:
    if (immediate > 2 ** 8 - 1 or immediate < -(2 ** 7)):
        raise AssemblerSyntaxError(lineNo,"immediate out of range")
  else:
    if signed and (immediate > 2 ** 7 - 1 or immediate < -(2 ** 7)):
        raise AssemblerSyntaxError(lineNo,"signed immediate out of range")
    if (not signed) and (immediate > 2 ** 8 - 1 or immediate < 0):
...
def assemble_instructions(inputFile):
  ...
  for line in inputFile:
    ...
    twoReg   = twoReg_re.match(instruction)
    ...
    if len(instruction) != 0:
      ...
      if rtype:
        ...
        rs = int(twoReg.group('rs')[2])
        rt = int(twoReg.group('rt')[2])
        opcode = opcodes[twoReg.group('instr')]
        num = opcode << 12 | rs << 10 | rt << 8
        debug("instruction: %s rs: %d rt: %d opcode: %d num: %04x" % (instruction,rs,rt,opcode,num))
      elif lui:
